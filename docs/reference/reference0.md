...
footer-left: Version 2.3.3+45ae59b
...
For Z3S5 Lisp Version 2.3.3+45ae59b with installed modules (tasks help beep db fileio decimal ling float console base).
...
Load the zimage file `fi` at optional path selector `sel`, if possible, and return a list containing information about the zimage after it has been loaded. If the zimage fails the semantic version check, then an error is raised.
...
Returns the non-nil entry-point of the zimage if the the zimage in file `fi` with optional path selector `sel` can be run, nil otherwise.
...
Read all data from stream with numeric ID `stream` and return it as an sexpr.
