...
footer-left: Version 2.4+507929c-gui.fyne2
author: by Erich Rast and all Help system contributors
date: 2023-9-28 16:15
header-includes: |
    \lstset{% for listings
...
For Z3S5 Lisp Version 2.4+507929c-gui.fyne2 with installed modules (gui fyne2 oop lib kvdb zimage tasks help beep db fileio decimal ling float console base).
...
Add the given `shortcut` to the given `canvas`, calling the handler `proc` when it is triggered. `proc` must be a function that takes a shortcut as argument.
...
Remove the `shortcut` from `canvas.`
